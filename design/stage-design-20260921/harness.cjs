...
const html=fs.readFileSync(path.join(__dirname,'baseline.html'),'utf8');
const source=html.match(/<script>([\s\S]*?)<\/script>/)[1].split('// ============ 启动 ============')[0];
const results=[];
function make(seedValue=29,baseMultiplier=1,costGrowth=1.25,gatePreset=false){
 ...
 let variant=source.replace("const ATK0=8, HP0=100,",`const ATK0=${8*baseMultiplier}, HP0=${100*baseMultiplier},`).replace('COST_G=1.25',`COST_G=${costGrowth}`);
 if(gatePreset)variant=variant.replace('tianshu?10:BOSS.hp','tianshu?8:BOSS.hp').replace('tianshu?3:BOSS.atk','tianshu?2:BOSS.atk').replace('tianshu?1.0:','tianshu?0.8:');
 vm.runInContext(variant,ctx);vm.runInContext('toast=floatDmg=lunge=enemyAnim=enemyDie=()=>{};S=freshState();newSession();spawnMob();',ctx);
