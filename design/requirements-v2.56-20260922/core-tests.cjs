...
test('C09','战斗','狂暴节奏暴击增益按游戏时间到期',g=>{g.run('Math.random=()=>0;S.skills=["critsurge"];cur.hp=100;dealPlayerHit()');near(g.run('calcStats().aspd'),1.3*1.3);g.run("combatTick+=181");near(g.run('calcStats().aspd'),1.3*1.3);g.run("combatTick+=120");near(g.run('calcStats().aspd'),1.3);});
