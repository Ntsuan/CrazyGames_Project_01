...
(function(root){
  ...
  const NAMES=['capsule','stage02-handoff','stage03-handoff','stage02-walk','stage03-walk','weapon02','weapon03','tool-arm'];
  ...
  function backdrop(ctx){
    const bg=ctx.createLinearGradient(0,0,0,HEIGHT);bg.addColorStop(0,'#080f1b');bg.addColorStop(1,'#11212b');ctx.fillStyle=bg;ctx.fillRect(0,0,WIDTH,HEIGHT);
    ctx.strokeStyle='rgba(76,130,158,.1)';ctx.lineWidth=1;
    for(let x=0;x<WIDTH;x+=80){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,HEIGHT);ctx.stroke()}
    ctx.fillStyle='#101a24';ctx.fillRect(0,615,WIDTH,105);
    ctx.fillStyle='rgba(93,167,187,.24)';ctx.fillRect(24,614,912,2);
    const g=ctx.createRadialGradient(465,480,40,465,480,390);g.addColorStop(0,'rgba(0,218,229,.07)');g.addColorStop(1,'rgba(0,218,229,0)');ctx.fillStyle=g;ctx.fillRect(0,0,WIDTH,HEIGHT);
  }
  function render(ctx,assets,time,options={}){
    ...
    let x=INSIDE, sheet=assets['stage02-handoff'],cols=2,index=3;
    if(t<1){x=mix(200,INSIDE,t);sheet=assets['stage02-walk'];cols=2;index=Math.floor(realTime/.225)%4}
    else if(t<1.45){index=Math.min(3,Math.floor((t-1)/.1125))}
    else if(t<4.25){sheet=assets[upgraded?'stage03-handoff':'stage02-handoff'];index=upgraded?0:3}
    else if(t<4.7){sheet=assets['stage03-handoff'];index=Math.min(3,Math.floor((t-4.25)/.1125))}
    else if(t<5.7){x=mix(INSIDE,740,t-4.7);sheet=assets['stage03-walk'];cols=2;index=Math.floor((realTime-5.5)/.225)%4}
    else {x=740;sheet=assets['stage03-handoff'];index=3}
    ...
    if(t>=1.3375 && t<4.3625){
      ...
      weapon(ctx,assets[upgraded?'weapon03':'weapon02'],wx,wy,wh);
      if(t<1.7 || t>=4){
  ...
  const api={WIDTH,HEIGHT,DURATION,UPGRADE_TIME,NAMES,render,loadAssets,phaseAt,Player};
  if(typeof module!=='undefined'&&module.exports)module.exports=api;
  else root.NeonUpgrade0203=api;
