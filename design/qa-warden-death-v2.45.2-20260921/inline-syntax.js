...
function onKill(){
  if(S.prestigeOffer) return;   // 已结算掉落的里程碑/受阻选择期间不再结算击杀
  ...
  if(boss){
    ...
    afterPresentation(prismKill?1100:900,()=>{
      ...
      afterPresentation(300,()=>{
        if(willSkill){ pendingTransit=true; if(S.prestigeOffer) showPrestigeOffer(); else offerSkills("p10",null,S.floor-1); }
...
function offerSkills(poolKey,preset,forFloor){ forFloor=forFloor??(S.pendingOffer&&S.pendingOffer.floor)??S.floor;   // v2.31.2：缺省从待选意图读楼层（C60）
  ...
  showModal(`<img class="ic" style="width:26px;height:26px;vertical-align:-6px;margin-right:9px" src="assets/icons/skill-choice.svg" alt="">神经技能 · 三选一`,   // v2.30：⚡/🎯 → skill-choice.svg
    ...
    picks.map(k=>`<button class="nh-button nh-button--upgrade" onclick="pickSkill('${k}')"><span class="nh-button__body"><span class="nh-button__title">${SKILLS[k].n}</span><span class="nh-button__detail">${k==="ghoststep"&&count(k)>0?"无敌持续时间增加 0.5 秒":SKILLS[k].d}</span></span></button>`).join(""));
}
...
function showPrestigeOffer(message=""){
  ...
  showModal("确认重塑",
    `<div class="prestige-title">选择本轮碎片结算方式</div>
     ...
     <div class="prestige-rules">回到第 1 层，重置强化与本轮技能<br>装备回收，保留永久加成与已有碎片<br>本轮信用点不带入</div>
...
function continueMilestone(){
  ...
  offerSkills(po.pool,po.picks,po.floor);   // 保留候选与继续意图，一并保存
}
function doPrestige(){
...
function applyPrestige(mult){
  ...
  try{ localStorage.setItem("neonHunter",JSON.stringify(next)); }
  ...
  buff={adrenaline:0,killcycle:0,critsurge:0,soulrush:0}; player.pendingInvuln=0; player.invulnUntil=0;
...
function loop(now){
  ...
  if(boot.phase!=="playing"){ requestAnimationFrame(loop); return; } // 封面／加载期间冻结战斗和计时
...
async function startCoverLoad(reset=false){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    setTimeout(()=>{
      cover.remove(); document.body.classList.remove("booting"); boot.phase="playing"; lastT=performance.now();
      for(const id of ["topbar","arena","panels"]) if($(id)) $(id).inert=false;
      if(boot.resetRequested) doReset(); else openingRunIn();
      if(!S.prestigeOffer){$("btnSpeed").focus({preventScroll:true});}
...
const VERSION="v2.45.2";   // Codex：典狱长死亡改为连续向左前方倒地
...
function askReset(){
  if(prestigeAd) return;   // 广告模拟界面通过取消返回，避免覆盖活动请求
...
function doReset(){
  ...
  prestigeAd=null; adRelease(); pendingTransit=false; S.prestigeOffer=null;
...
// 重塑弹窗内循环键盘焦点，避免 Tab 穿到背景改变冻结后的装备/资源。
document.addEventListener("keydown",e=>{
  if(e.key!=="Tab"||!S.prestigeOffer||!$("overlay").classList.contains("show")) return;
...
$("btnLoadRetry").onclick=()=>startCoverLoad(boot.resetRequested);
function askCoverReset(){
  if(boot.phase!=="cover") return;
  showModal("⚠️ 清除存档","确定要清空全部进度吗？此操作不可恢复。",
    `<button class="nh-button nh-button--danger nh-button--compact" onclick="closeModal();startCoverLoad(true)">确认清除并开始新游戏</button>
     <button onclick="closeModal()">取消</button>`);
}
// 禁止键盘穿透封面；设置里的日志和确认弹窗仍可正常操作。
for(const id of ["topbar","arena","panels"]) if($(id)) $(id).inert=true;
prepareAssets(); // 封面预热；点击时复用正在加载和已就绪的资源。
