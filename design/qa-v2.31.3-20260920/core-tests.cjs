...
test('C40','日志','事件包括版本和数值快照',g=>{g.run('S.gold=6;$("btnAtk").onclick()');assert.equal(g.run('curSession.build'),'v2.31.3');assert(g.run('curSession.events.some(x=>x.type==="upgrade"&&x.snap.atkLv===1)'));});
...
const report={version:'v2.31.3',sourceHash:crypto.createHash('sha256').update(html).digest('hex'),environment:'Node VM; actual game functions with DOM/audio stubs, virtual clock; not a browser compatibility test',count:results.length,passed:results.filter(r=>r.status==='PASS').length,failed:results.filter(r=>r.status==='FAIL').length,results};
