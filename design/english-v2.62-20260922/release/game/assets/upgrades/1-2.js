...
(function(root){
  ...
  const NAMES=['capsule','stage01-handoff','stage02-handoff','stage01-walk','stage02-walk','weapon01','weapon02','tool-arm','repair-room'];
  ...
  function phaseAt(time){
    ...
    if(t<1.7)return L('武器交接');
    if(t<2)return L('舱门闭合');
    if(t<3.65)return L('义体与武器升级');
    if(t<4)return L('改造完成 · 开舱');
    if(t<4.7)return L('接回升级步枪');
  ...
  function render(ctx,assets,time,options={}){
    ...
    let x=INSIDE, sheet=assets['stage01-handoff'],cols=2,index=3;
    if(t<1){x=mix(200,INSIDE,t);sheet=assets['stage01-walk'];cols=2;index=Math.floor(realTime/.225)%4}
    else if(t<1.45){index=Math.min(3,Math.floor((t-1)/.1125))}
    else if(t<4.25){sheet=assets[upgraded?'stage02-handoff':'stage01-handoff'];index=upgraded?0:3}
    else if(t<4.7){sheet=assets['stage02-handoff'];index=Math.min(3,Math.floor((t-4.25)/.1125))}
    else if(t<5.7){x=mix(INSIDE,740,t-4.7);sheet=assets['stage02-walk'];cols=2;index=Math.floor((realTime-5.5)/.225)%4}
    else {x=740;sheet=assets['stage02-handoff'];index=3}
    ...
    if(t>=1.3375 && t<4.3625){
      ...
      weapon(ctx,assets[upgraded?'weapon02':'weapon01'],wx,wy,wh);
      if(t<1.7 || t>=4){
