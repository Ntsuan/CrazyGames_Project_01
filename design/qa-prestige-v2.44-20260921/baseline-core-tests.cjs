...
test('C57','技能','里程碑弹窗使用刚完成的层数',g=>{g.run('S.floor=10;S.mobIdx=5;spawnMob();onKill()');g.advance(1500);assert(g.run('$("modalBox").innerHTML.includes("突破第 10 层")'));});
...
test('C67','弹窗','挂起技能在取消复位后补弹',g=>{g.run('S.floor=10;S.mobIdx=5;spawnMob();onKill();askReset()');g.advance(1500);g.run('confirmOpen=false;closeModal()');g.advance(100);assert(g.run('skillOfferOpen&&$("modalBox").innerHTML.includes("突破第 10 层")'));});
...
test('C72','弹窗','等待复位遇死亡后重塑不应吞掉未来技能',g=>{g.run('askReset();player.hp=0;checkWall(.1);applyPrestige(1)');g.advance(1600);g.run('S.floor=5;S.mobIdx=5;spawnMob();onKill()');g.advance(1300);assert(g.run('skillOfferOpen'),JSON.stringify(g.json('({confirmOpen,skillOfferOpen,pendingSkillOffer,curHp:cur.hp})')));});
