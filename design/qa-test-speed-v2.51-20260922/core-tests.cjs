...
test('C39','补给','测试版无限补给按当前设计发奖',g=>{g.run('openCrate();openCrate()');assert.equal(g.run('curSession.events.filter(x=>x.type==="crate").length'),2);assert.equal(g.run('S.gold'),313);});
