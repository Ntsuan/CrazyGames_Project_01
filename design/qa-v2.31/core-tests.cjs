...
test('C40','日志','事件包括版本和数值快照',g=>{g.run('S.gold=6;$("btnAtk").onclick()');assert.equal(g.run('curSession.build'),'v2.30.11');assert(g.run('curSession.events.some(x=>x.type==="upgrade"&&x.snap.atkLv===1)'));});
...
test('C54','重塑','死亡重塑即时存档应为新局满血',g=>{g.run('player.hp=0;wall=true;paused=true;applyPrestige(1)');assert.equal(g.json('JSON.parse(localStorage.getItem("neonHunter")).playerHp'),100);});
...
test('C61','存档','读档初始刷怪和跑入两阶段均保持存档血量',g=>{g.run('player.hp=33;save();load();spawnMob();openingRunIn()');g.advance(1600);assert.equal(g.run('player.hp'),33);});
...
const report={version:'v2.30.11',sourceHash:crypto.createHash('sha256').update(html).digest('hex'),environment:'Node VM; actual game functions with DOM/audio stubs, virtual clock; not a browser compatibility test',count:results.length,passed:results.filter(r=>r.status==='PASS').length,failed:results.filter(r=>r.status==='FAIL').length,results};
