...
def request_file(r):
    try:
        with urllib.request.urlopen('http://127.0.0.1:19319/'+r['file']) as response:
...
report={'assetCount':len(results),'assetFailures':sum(r['status']=='FAIL' for r in results),'staticReferences':len(refs),'missingStaticReferences':missing,'duplicateIds':[k for k,v in Counter(parser.ids).items() if v>1],'results':results}
