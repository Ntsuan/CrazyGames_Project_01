...
const source=html.match(/<script>([\s\S]*?)<\/script>/)[1].split('// ============ 启动 ============')[0]+'\n'+html.match(/function askCoverReset\(\)\{[\s\S]*?\n\}/)[0];
const results=[];
function make(seedValue=29,initialStorage=new Map(),profile="development",locale=null){
 let now=100000,nextId=0,timers=[],seed=seedValue;const storage=initialStorage,nodes=new Map();
 ...
 const ctx=vm.createContext({console,Math:math,Date:FakeDate,performance:{now:()=>now},Image:function(){},Audio:function(){this.play=()=>Promise.resolve();},document:{body:element(),listenerCount:0,hidden:false,getElementById(id){if(!nodes.has(id))nodes.set(id,element());return nodes.get(id);},createElement:element,addEventListener(...a){this.listenerCount++}},window:{innerWidth:390,reloads:0,location:{reload(){ctx.window.reloads++;}},addEventListener(){}},localStorage:{getItem:k=>storage.get(k)||null,setItem:(k,v)=>storage.set(k,String(v)),removeItem:k=>storage.delete(k)},setTimeout(fn,delay){timers.push({fn,at:now+(delay||0),id:++nextId});return nextId;},clearTimeout(id){timers=timers.filter(x=>x.id!==id);},requestAnimationFrame(){},cancelAnimationFrame(){},getComputedStyle:()=>({backgroundImage:''}),URL,Blob});
 if(locale){ctx.document.documentElement={setAttribute(){}};ctx.window.localStorage=ctx.localStorage;ctx.window.location.href='https://example.test/?lang='+locale;ctx.window.NeonEnglish=JSON.parse(fs.readFileSync(path.join(__dirname,'../../game/assets/i18n/en.json'),'utf8'));vm.runInContext(fs.readFileSync(path.join(__dirname,'../../game/assets/i18n/runtime.js'),'utf8'),ctx);}
 vm.runInContext(source.replace(/const BUILD_PROFILE="[^"]+";/,`const BUILD_PROFILE=${JSON.stringify(profile)};`),ctx);vm.runInContext('if(typeof boot!=="undefined")boot.phase="playing";__realLunge=lunge;toast=floatDmg=lunge=enemyAnim=enemyDie=()=>{};S=freshState();newSession();spawnMob();',ctx);
