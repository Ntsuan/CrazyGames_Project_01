...
const html=fs.readFileSync(process.env.GAME_HTML||path.join(__dirname,'../../game/index.html'),'utf8');
const source=html.match(/<script>([\s\S]*?)<\/script>/)[1].split('// ============ 启动 ============')[0].replace(/const BUILD_PROFILE="[^"]+";/,'const BUILD_PROFILE="development";');
const results=[];
function make(seedValue=29){
 ...
 vm.runInContext(source,ctx);vm.runInContext('if(typeof boot!=="undefined")boot.phase="playing";toast=floatDmg=lunge=enemyAnim=enemyDie=()=>{};S=freshState();newSession();spawnMob();',ctx);
...
test('C06','战斗','普通 Boss/棱镜/天枢血盾参数',g=>{for(const [f,hp,atk,shield] of [[1,6,1.8,.5],[10,6,1.8,.8],[100,8,2,.8]]){g.run(`S.floor=${f};S.mobIdx=5;spawnMob()`);near(g.run('cur.max'),20*1.15**f*hp);near(g.run('cur.atk'),1.5*1.15**f*atk);near(g.run('cur.maxShield'),Math.round(20*1.15**f*hp*shield));}});
test('C07','战斗','盾伤不吸血，破盾余伤吸血并触发猎手直觉',g=>{g.run('Math.random=()=>1;S.skills=["nano","soulrush"];player.hp=50;cur.hp=100;cur.shield=10;dealPlayerHit()');near(g.run('player.hp'),50);g.run('dealPlayerHit()');near(g.run('player.hp'),50.3);assert(g.run('buff.soulrush>combatNow()'));});
...
test('C09','战斗','狂暴节奏暴击增益按技能实战时间到期',g=>{g.run('Math.random=()=>0;S.skills=["critsurge"];cur.hp=100;dealPlayerHit()');near(g.run('calcStats().aspd'),1.3*1.3);g.run("skillTick+=181");near(g.run('calcStats().aspd'),1.3*1.3);g.run("skillTick+=120");near(g.run('calcStats().aspd'),1.3);});
test('C10','战斗','死亡进入暂停与重塑弹窗',g=>{g.run('player.hp=0;checkWall(.1)');assert(g.run('wall&&paused'));g.advance(1200);assert(g.run('$("modalBox").innerHTML.includes("重塑")'));});
...
test('C22','装备','民用及以下无词条，最高品质保留原值',g=>{assert.equal(g.run('affixOf("weapon",2)'),null);assert.deepStrictEqual(g.json('affixOf("weapon",9)'),{t:'crit',v:8});assert.deepStrictEqual(g.json('affixOf("implant",9)'),{t:'aspd',v:16});assert.deepStrictEqual(g.json('affixOf("armor",9)'),{t:'ls',v:6});});
...
test('C33','重塑','结算碎片、15%回收并清除局内成长',g=>{g.run('S.floor=20;S.maxFloor=19;S.bestEver=19;S.runSouls=22;S.souls=3;S.tree.dmg=2;S.equip.weapon={slot:"weapon",q:3};S.skills=["nano"];S.atkLv=12');const gold=g.run('salvageAllGold().g');g.run('wall=true;doPrestige();applyPrestige(1)');assert.deepStrictEqual(g.json('[S.souls,S.floor,S.atkLv,S.skills.length,S.bestEver,S.tree.dmg,S.gold]'),[25,1,0,0,19,2,gold]);});
test('C34','重塑','广告双倍结算短期重复调用只发一次',g=>{g.run('S.runSouls=10;wall=true;doPrestige();requestPrestigeAd();finishPrestigeAd(prestigeAd.attempt,"completed");applyPrestige(2)');assert.equal(g.run('S.souls'),20);assert.equal(g.run('S.prestiges'),1);});
test('C35','重塑','结算后应立即持久化而非等待跑入',g=>{g.run('S.runSouls=10;save();wall=true;doPrestige();applyPrestige(1)');assert.equal(g.json('JSON.parse(localStorage.getItem("neonHunter")).prestiges'),1,'刚结算时持久化仍是旧循环');});
...
test('C39','补给','60秒冷却与固定20倍奖励',g=>{g.run('openCrate()');assert.equal(g.run('S.gold'),0);g.run('S.crateCooldownMs=0;paused=false;openCrate();finishCrateAd(crateAd.id,"completed");openCrate()');assert.equal(g.run('curSession.events.filter(x=>x.type==="crate").length'),1);assert.equal(g.run('S.gold'),46);});
test('C40','日志','事件包括版本和数值快照',g=>{g.run('S.gold=6;$("btnAtk").onclick()');assert.equal(g.run('curSession.build'),g.run('VERSION'));assert(g.run('curSession.events.some(x=>x.type==="upgrade"&&x.snap.atkLv===1)'));});
...
test('C54','重塑','死亡重塑即时保存奖励且运行态满血',g=>{g.run('player.hp=0;wall=true;paused=true;doPrestige();applyPrestige(1)');assert.equal(g.run('player.hp'),100);assert.equal(g.json('JSON.parse(localStorage.getItem("neonHunter")).prestiges'),1);});
...
test('C57','重塑','里程碑先显示刚完成层数的重塑选择',g=>{g.run('S.floor=10;S.mobIdx=5;spawnMob();onKill()');g.advance(1500);assert(g.run('$("modalBox").innerHTML.includes("第 10 层已通关")'));});
...
test('C62','重塑','死亡重塑跑入期间自动保存不会导致再死亡',g=>{g.run('player.hp=0;wall=true;paused=true;doPrestige();applyPrestige(1)');g.advance(500);g.run('save()');g.advance(1000);g.run('lastT=performance.now()-16;loop(performance.now())');assert(g.run('player.hp>0&&!wall'),JSON.stringify(g.json('({hp:player.hp,wall})')));});
...
test('C67','弹窗','取消复位恢复里程碑，继续后再选技能',g=>{g.run('S.floor=10;S.mobIdx=5;spawnMob();onKill();askReset()');g.advance(1500);g.run('cancelReset();continueMilestone()');g.advance(100);assert(g.run('skillOfferOpen&&$("modalBox").innerHTML.includes("突破第 10 层")'));});
test('C68','弹窗','挂起技能在确认复位后废弃',g=>{g.run('S.floor=10;S.mobIdx=5;spawnMob();onKill();askReset()');g.advance(1500);g.run('doReset()');g.advance(2000);assert(g.run('S.floor===1&&!skillOfferOpen&&!pendingSkillOffer&&!S.pendingOffer'));});


test('C69','存档','回本层起点但金币碎片装备与技能不回退',g=>{g.run('S.floor=12;S.mobIdx=4;S.gold=123;S.runSouls=11;S.souls=7;S.skills=["fire"];S.equip.weapon={slot:"weapon",q:4};player.hp=20;save();load();spawnMob()');assert.deepStrictEqual(g.json('[S.floor,S.mobIdx,player.hp,S.gold,S.runSouls,S.souls,S.skills[0],S.equip.weapon.q]'),[12,0,100,123,11,7,'fire',4]);});
test('C70','UI','有盾角色跑入期间血盾条同时隐藏',g=>{g.run('S.skills=["shield"];spawnMob();openingRunIn();renderCombat()');assert(g.run('$("playerHp").style.display==="none"&&$("playerSh").style.display==="none"'));g.advance(1600);g.run('renderCombat()');assert(g.run('$("playerSh").style.display==="block"'));});
test('C71','弹窗','确认复位等待期间死亡不得覆盖确认或遗留确认标记',g=>{g.run('askReset();player.hp=0;checkWall(.1)');assert(g.run('!confirmOpen||$("modalBox").innerHTML.includes("确认复位")'),JSON.stringify(g.json('({confirmOpen,wall,dialog:$("modalBox").innerHTML})')));});
test('C72','弹窗','等待复位遇死亡后重塑不应吞掉未来技能',g=>{g.run('askReset();player.hp=0;checkWall(.1);doPrestige();applyPrestige(1)');g.advance(1600);g.run('S.floor=5;S.mobIdx=5;spawnMob();onKill()');g.advance(1300);assert(g.run('skillOfferOpen'),JSON.stringify(g.json('({confirmOpen,skillOfferOpen,pendingSkillOffer,curHp:cur.hp})')));});


test('C73','日志','跨版本升级不把新行为写入旧版本测试段',g=>{g.run('curSession.build="v2.32";saveLogs(true);curSession=null;loadLogs();if(!curSession)newSession();S.gold=6;$("btnAtk").onclick()');assert.equal(g.run('curSession.build'),g.run('VERSION'));});
test('C74','存储','首次启动存储已满时初始化不得抛异常',g=>{g.storage.clear();g.run('curSession=null;lastLogSave=0;localStorage.setItem=()=>{throw Error("QuotaExceededError")};loadLogs();if(!curSession)newSession()');});


test('C75','日志','跨版本自动封存仍只保留最近三段',g=>{g.run('sessions=Array.from({length:3},(_,i)=>({...curSession,id:"S"+i,endedAt:Date.now()}));curSession.build="v2.33";saveLogs(true);curSession=null;loadLogs();if(!curSession)newSession()');assert.equal(g.run('sessions.length'),3);});
test('C76','存储','日志存储满时升级仍生效且内存记录存在',g=>{g.run('S.gold=6;lastLogSave=0;localStorage.setItem=()=>{throw Error("QuotaExceededError")};$("btnAtk").onclick()');assert.equal(g.run('S.atkLv'),1);assert(g.run('curSession.events.some(x=>x.type==="upgrade")'));});

const report={version:html.match(/const VERSION="([^"]+)"/)[1],sourceHash:crypto.createHash('sha256').update(html).digest('hex'),environment:'Node VM; actual game functions with DOM/audio stubs, virtual clock; not a browser compatibility test',count:results.length,passed:results.filter(r=>r.status==='PASS').length,failed:results.filter(r=>r.status==='FAIL').length,results};
fs.writeFileSync(path.join(__dirname,(process.env.OUTPUT_TAG||'current')+'-core-results.json'),JSON.stringify(report,null,2));
...
// Known baseline issue C75 is recorded, not silently counted as a pass.
if(results.some(r=>r.status==="FAIL"&&!(r.id==="C75"&&r.error==="4 == 3")))process.exitCode=1;
