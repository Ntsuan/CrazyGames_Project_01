...
(function(root){
  ...
  const NAMES=['capsule','stage04-handoff','stage05-activation','stage04-walk','stage05-walk','weapon04','tool-arm','repair-room'];
  ...
  function phaseAt(time){
    ...
    if(t<1)return L('进入修复舱');
    if(t<1.7)return L('脉冲枪送入回收架');
    if(t<2)return L('舱门闭合');
    if(t<3.65)return L('全身义体重构 · 臂炮融合');
    if(t<4)return L('改造完成 · 开舱');
    if(t<4.7)return L('臂炮校准 · 核心点亮');
    if(t<5.7)return L('新阶段出舱');
    return L('05 阶段 · 全身义体');
  }
  ...
  function render(ctx,assets,time,options={}){
    ...
    let x=INSIDE, sheet=assets['stage04-handoff'],cols=2,index=3;
    if(t<1){x=mix(200,INSIDE,t);sheet=assets['stage04-walk'];cols=2;index=Math.floor(realTime/.225)%4}
    else if(t<1.45){index=Math.min(3,Math.floor((t-1)/.1125))}
    else if(t<4){sheet=assets[upgraded?'stage05-activation':'stage04-handoff'];index=upgraded?0:3}
    else if(t<4.7){sheet=assets['stage05-activation'];index=Math.min(3,Math.floor((t-4)/.175))}
    else if(t<5.7){x=mix(INSIDE,740,t-4.7);sheet=assets['stage05-walk'];cols=2;index=Math.floor((realTime-5.5)/.225)%4}
    else {x=740;sheet=assets['stage05-activation'];index=3}

    // The old pulse rifle remains on the rack. Stage05 has an integrated cannon.
    if(t>=1.3375){
      let wx=654,wy=318,wh=198;
      if(t<1.7){const p=ease((t-1.3375)/.3625);wx=mix(542,654,p);wy=mix(186,318,p);wh=mix(202,198,p)}
      weapon(ctx,assets.weapon04,wx,wy,wh,upgraded?.65:1);
      if(t<1.7){ctx.save();ctx.strokeStyle='#69ecf1';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(642,446);ctx.lineTo(wx+5,wy+wh*.65);ctx.stroke();ctx.restore()}
    }
    ...
    if(t>=1.7 && t<4){
      ...
      tool(ctx,assets['tool-arm'],550,294,176,-a,-1);
      // A service arm targets the forearm socket while the other works at the head/neck.
      ctx.restore();
    }
    if(closed>0){
    ...
    if(t>=2 && t<3.65){
      ...
      ctx.globalAlpha=1;
      ctx.restore();
      // Local forearm alignment scan; no upgrade is applied to the retired rifle.
      ctx.save();bayClip(ctx);ctx.strokeStyle='#7dfbff';ctx.lineWidth=2;ctx.globalAlpha=.55+.25*Math.sin(p*8);
      ctx.beginPath();ctx.ellipse(442,425,43,68,0,0,Math.PI*2);ctx.stroke();ctx.restore();
    }
  ...
  async function loadAssets(base='.',tier='desktop',loader){
    const load=loader || (src=>new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>resolve(im);im.onerror=()=>reject(new Error(L('无法加载素材：')+src));im.src=src}));
  ...
  const api={SOURCE_STAGE:4,TARGET_STAGE:5,WIDTH,HEIGHT,DURATION,UPGRADE_TIME,NAMES,render,loadAssets,phaseAt,Player};
  if(typeof module!=='undefined'&&module.exports)module.exports=api;
  else root.NeonUpgrade0405=api;
